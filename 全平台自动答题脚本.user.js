...
(function() {
    ...
    async function searchAnswerByOpenAI(data) {
        ...
        let prompt = "你是一个答题助手，请直接给出答案，不要解释。\n";
        prompt += "题型：" + typeName + "\n";
        prompt += "题目：" + data.question + "\n";
        ...
        if (typeName === "判断题") {
            prompt += "请只回答：正确 或 错误\n";
        } else if (typeName === "单选题") {
            prompt += "请只回答一个选项字母（如 A）\n";
        } else if (typeName === "多选题") {
            prompt += "请回答所有正确选项的字母（如 ABC）\n";
        } else if (typeName === "填空题") {
            prompt += "请直接给出填空答案，多个空用 | 分隔\n";
        } else {
            prompt += "请简洁作答\n";
        }
                ...
                onload: function(r) {
                    ...
                    try {
                        ...
                        if (typeName === "单选题" || typeName === "判断题") {
                            ...
                            answers = answerText.split("|").map(s => s.trim());
                        } else {
                            answers = [answerText];
                        }
...
<head>
    ...
    <style>
      ...
      * { margin: 0; padding: 0; box-sizing: border-box; }
      ...
      .btn-skip:hover { border-color: var(--accent); }
      .btn-pay { background: var(--accent); color: #fff; }
      .btn-pay:hover { background: var(--accent-hover); }
      .btn-pay.active { background: var(--bg-card); border: 1px solid var(--accent); }
      .btn-credit { background: linear-gradient(135deg, #667eea, #764ba2); color: #fff; text-decoration: none; display: inline-block; }
      .btn-credit:hover { filter: brightness(1.1); }
      .btn-setting { background: var(--bg-card); color: var(--text-primary); border: 1px solid var(--border); margin-left: auto; }
      .btn-setting:hover { border-color: var(--accent); }
      .btn-auto { background: var(--bg-card); color: var(--text-primary); border: 1px solid var(--border); }
      .btn-auto:hover { border-color: var(--warning); }
      .btn-auto.active { background: var(--warning); color: #000; }
      .result-area { flex: 1; overflow-y: auto; padding: 0; }
      .result-area::-webkit-scrollbar { width: 4px; }
      .result-area::-webkit-scrollbar-track { background: var(--bg-primary); }
      ...
      .result-table tr:hover { background: rgba(233,69,96,0.05); }
      .result-table .col-idx { width: 36px; text-align: center; color: var(--text-secondary); }
      .result-table .col-q { width: 45%; }
      .result-table .col-a { width: 45%; }
      ...
      .modal-box::-webkit-scrollbar-thumb { background: var(--bg-card); border-radius: 2px; }
      .modal-header {
      ...
      .form-group .hint { font-size: 10px; color: var(--text-secondary); margin-top: 3px; }
      .form-row {
        display: flex; align-items: center; justify-content: space-between;
        margin-bottom: 14px;
      }
      ...
      .modal-footer .btn-save:hover { background: var(--accent-hover); }
      .similar-overlay {
      ...
      .similar-box h4 { font-size: 13px; margin-bottom: 10px; }
      .similar-box .content { font-size: 11px; color: var(--text-secondary); line-height: 1.6; }
    </style>
</head>
<body>
<div id="app">
  <div class="modal-overlay" v-if="show_setting" @click.self="show_setting=false">
    <div class="modal-box">
      ...
      <div class="modal-body">
        ...
        <div class="form-group">
          <label>题库适配器 URL</label>
          <input v-model="tiku_adapter" placeholder="tikuAdapter 地址">
          <div class="hint">聚合多家题库，更精确查题</div>
        </div>
        <div class="section-title">AI 答题（OpenAI 兼容接口）</div>
        <div class="form-row">
          <label>启用 AI 答题</label>
          <label class="toggle">
            <input type="checkbox" v-model="openai_enabled">
  ...
  <div class="similar-overlay" v-if="showSimilar" @click.self="showSimilar=false">
    <div class="similar-box">
      <h4>相似答案</h4>
      <div class="content" v-html="similarContent"></div>
    </div>
  </div>
  <div style="display:flex;flex-direction:column;height:100%;">
    ...
    <div class="action-bar" v-if="!hidden">
      ...
      <button class="btn btn-pay" :class="{active: opt.start_pay}" @click="btnClick(null,'opt.start_pay')">
        {{ opt.start_pay ? '关闭收费' : '开启收费' }}
      </button>
      <a class="btn btn-credit" href="https://lyck6.cn/pay" target="_blank">获取积分</a>
      ...
      <table class="result-table" v-if="tableData.length">
        ...
        <tbody>
          <tr v-for="row in tableData" :key="row.index" :class="rowClass(row)">
            ...
            <td class="col-q" v-html="row.question"></td>
            <td class="col-a">
    ...
    methods: {
        save_setting(){
            window.parent.postMessage({
                ...
                gpt: this.gpt,
                tiku_adapter: this.tiku_adapter,
                openai_enabled: this.openai_enabled,
        ...
        rowClass(row){
            if (row.style === 'success-row') return 'row-success'
            if (row.style === 'warning-row') return 'row-warning'
            if (row.style === 'primary-row') return 'row-success'
            return ''
        },
        viewSimilar(row){
...
`;
    ...
    function parseYxbyunExam(problems) {
        return problems.map(item => {
            const type = getQuestionType(item.dtmc);
            return item.sjSjstSaveReqVOS.map(item => {
                let answer = [];
                let options = [];
                if (type === 2) {
                    return {
                        question: formatString(item.tg),
                        options: options,
                        type: type,
                        answer: answer
                    };
                } else if (type === 0 || type === 3 || type === 1) {
                    let answer = item?.dxpdtda ? item?.dxpdtda.split(",") : item?.dxtda ? item?.dxtda.split(",") : [];
                    let options = [];
                    for (let subjectOption of item?.tkStxxSaveReqVOS || []) {
                        const opt = formatString(subjectOption.xxnr);
                        options.push(opt);
                    }
                    return {
                        question: formatString(item.tg),
    ...
    WorkerJSPlus({
        name: "柠檬文才独立考试",
        match: () => {
            const pathMatch = location.hash.includes("/exam?studentId");
            const matchHostArr = [ "exam.wencaischool.net" ];
            return pathMatch && matchHostArr.includes(location.host);
        },
        intv: () => {
            return $("#paperExam").css("display") != "none" && $(".timenum").length;
        },
        root: ".paperWrapper .tmList .tmc",
        elements: {
            question: ".tmTitleTxt",
            options: ".ansbox .opCont",
            $options: ".ansbox input"
    ...
    WorkerJSPlus({
        name: "亿学宝云考试",
        match: location.host.includes("yxbyun.com") && location.href.includes("/exam"),
        intv: () => {
            return $(".datiqu").length;
        },
        hook: () => {
            JSONParseHook(o => {
                if (o?.data?.sjSjRespDto?.sjDtSaveReqVOS) {
                    GLOBAL.json = parseYxbyunExam(o?.data?.sjSjRespDto?.sjDtSaveReqVOS).reduce((acc, cur) => {
        ...
        elements: {
            question: ".tm",
            options: ".xuanxiang .xxnr",
            $options: ".xuanxiang .xxbh",
            type: ".tx"
        },
        wrap: obj => {
            Object.assign(obj, GLOBAL.json[GLOBAL.index - 1]);
